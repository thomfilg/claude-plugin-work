...
test('Stop-word trigger tokens raise R2 errors and exit non-zero', () => {
  ...
  assert.ok(STOP_WORDS.has('permission') && STOP_WORDS.has('hook') && STOP_WORDS.has('blocked'),
  ...
  const tokens = r2.map((e) => e.message).join(' ').toLowerCase();
...
test('R2 ignores multi-word phrases', () => {
  ...
  const manifest = {
    memories: [
      makeMemory({ name: 'phrase-mem', trigger_prompt: '\\b(git\\s+push|amend)\\b' }),
    ],
  };
  const result = runLint(manifest);
  const r2 = result.env.errors.filter((e) => e.rule === 'R2-stopword');
...
test('R7 auto-fixes inject=full when body exceeds 30 lines', () => {
  ...
  assert.equal(result.env.manifest.memories[0].inject, 'summary',
...
test('R4 errors when PreToolUse is configured without trigger_pretool', () => {
  ...
  const manifestEmpty = {
    memories: [
      makeMemory({ events: ['UserPromptSubmit', 'PreToolUse'], trigger_pretool: [] }),
    ],
  };
