...
let GIT_ROOT;
let EMPTY_SESSION_DIR;

function runHook(input, extraEnv = {}) {
  return new Promise((resolve, reject) => {
    const proc = spawn('node', [HOOK_PATH], {
      stdio: ['pipe', 'pipe', 'pipe'],
      env: { ...process.env, SESSION_GUARD_DIR: EMPTY_SESSION_DIR, ...extraEnv },
    });
    let stdout = '',
      stderr = '';
...
describe('enforce-completion-protocol hook', () => {
  before(() => {
    ...
    EMPTY_SESSION_DIR = path.join(os.tmpdir(), `test-empty-session-${process.pid}-${Date.now()}`);
    ...
    fs.mkdirSync(EMPTY_SESSION_DIR, { recursive: true });
  ...
  after(() => {
    ...
    fs.rmSync(EMPTY_SESSION_DIR, { recursive: true, force: true });
  });
  ...
  describe('work session bypass', () => {
    let sessionDir;
    let sessionFile;

    before(() => {
      sessionDir = path.join(os.tmpdir(), `test-session-dir-${process.pid}-${Date.now()}`);
    ...
    function writeSessionFile(data, suffix = 'test') {
      sessionFile = path.join(sessionDir, `claude-session-guard-${suffix}.json`);
      fs.writeFileSync(sessionFile, JSON.stringify(data));
      return sessionFile;
    }

    it('should APPROVE (exit 0) when active unrevealed /work session exists', async () => {
      writeSessionFile({ workflow: '/work', revealed: false, ticketId: 'GH-250' });
      const tp = createTranscript([
        {
          type: 'assistant',
          message: {
    ...
    it('should BLOCK when no session files exist', async () => {
      // Remove any session files
      const files = fs.readdirSync(sessionDir);
      for (const f of files) {
        if (f.startsWith('claude-session-guard-')) fs.unlinkSync(path.join(sessionDir, f));
      }
      const tp = createTranscript([
        {
          type: 'assistant',
    ...
    it('should BLOCK when session exists but revealed=true', async () => {
      writeSessionFile({ workflow: '/work', revealed: true, ticketId: 'GH-250' });
      const tp = createTranscript([
        {
          type: 'assistant',
          message: {
    ...
    it('should BLOCK when session exists but workflow is not /work', async () => {
      writeSessionFile({ workflow: '/check', revealed: false, ticketId: 'GH-250' });
      const tp = createTranscript([
        {
          type: 'assistant',
          message: {
    ...
    it('should BLOCK when session exists for different ticket', async () => {
      writeSessionFile({ workflow: '/work', revealed: false, ticketId: 'GH-999' });
      const tp = createTranscript([
        {
          type: 'assistant',
          message: {
