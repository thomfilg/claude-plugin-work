...
describe('transitionStep', () => {
  ...
  it('auto-completes workflow when reaching terminal step (targets=[])', () => {
    ...
    assert.strictEqual(
      ws.stepStatus['step_d'],
      'completed',
      'Terminal step should be auto-completed'
    );
  ...
  it('does NOT auto-complete when step has outgoing transitions', () => {
    ...
    assert.strictEqual(
      ws.stepStatus['step_b'],
      'in_progress',
      'Non-terminal step should stay in_progress'
    );
  ...
  it('rejects unknown step name', () => {
    ...
    assert.strictEqual(result.error, true);
    assert.ok(
      result.message.includes('Invalid step'),
      `Expected "Invalid step" in: ${result.message}`
    );
...
describe('onTransition callback', () => {
  ...
  it('rolls back transition when onTransition throws', () => {
    const wf = mockWorkflow({
      stateDir: stateDir,
      onTransition: () => {
        throw new Error('onTransition exploded');
      },
    });
    ...
    assert.strictEqual(
      ws.stepStatus['step_a'],
      'in_progress',
      'step_a should be rolled back to in_progress'
    );
    assert.strictEqual(
      ws.stepStatus['step_b'],
      'pending',
      'step_b (intermediate) should remain pending, not auto-completed'
    );
    assert.strictEqual(
      ws.stepStatus['step_c'],
      'pending',
      'step_c should be rolled back to pending'
    );
  ...
  it('rolls back terminal transition when onTransition throws (ws.status restored)', () => {
    const wf = mockWorkflow({
      stateDir: stateDir,
      onTransition: () => {
        throw new Error('terminal rollback test');
      },
    });
    ...
    assert.notStrictEqual(
      ws.status,
      'completed',
      'workflow status should NOT be completed after rollback'
    );
  });
