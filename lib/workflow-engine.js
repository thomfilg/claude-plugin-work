 ...
 *
 * Loads workflow definitions from plugin workflows/ and ~/.claude/workflows/ and provides:
...
// Scan both plugin workflows and ~/.claude/workflows (for non-plugin workflows like create-jira)
...
/** Scan plugin workflows/ and ~/.claude/workflows/ for *.workflow.js files */
...
function loadWorkflow(name) {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(`Workflow "${name}" not found in plugin or ~/.claude/workflows/`);
  }
...
function defaultFormatPlan(workflow, instanceId, plan, summary) {
  ...
  lines.push('');
  lines.push(`  SUMMARY: ${summary.run} RUN, ${summary.skip} SKIP, ${summary.pending} PENDING`);
  ...
  switch (command) {
    case 'plan': {
      ...
      const summary = {
        ...
        firstAction: byAction('RUN')[0]?.step || 'none',
