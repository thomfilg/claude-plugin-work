...
describe('hook-error-log', () => {
  ...
  beforeEach(() => {
    ...
    // Reset the cached _logFileSecured flag by re-requiring the module
  ...
  it('logHookError writes to the log file', () => {
  ...
  it('lines are truncated when exceeding MAX_LINE (3800 bytes)', () => {
    const { logHookError } = require('../hook-error-log');
    const longMessage = 'x'.repeat(5000);
    logHookError(__filename, new Error(longMessage));
    const content = fs.readFileSync(tmpFile, 'utf8');
    const line = content.trim();
    // 3800 chars + "..." = 3803, but the full line includes prefix
    assert.ok(line.length <= 3804, `line should be truncated (got ${line.length})`);
  ...
  it('ENFORCE_HOOK_DEBUG=1 writes to stderr instead of file', () => {
    ...
    // File should NOT exist or be empty (secureLogFile creates it, but no log line appended)
  ...
  it('secureLogFile creates file with 0o600 permissions', () => {
    const { logHookError } = require('../hook-error-log');
  ...
  it('symlinks are removed and recreated', () => {
