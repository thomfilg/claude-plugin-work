...
describe('work-actions', () => {
  ...
  describe('appendAction', () => {
    it('should create file and append a single action', () => {
      ...
      assert.strictEqual(actions[0].step, '1_ticket');
    ...
    it('should include meta when provided', () => {
      appendAction(TEST_TICKET, { step: '8_check', what: 'BLOCKED: Skill(check)', meta: { rule: 1 } });
  ...
  describe('analyzeActions', () => {
    ...
    it('should compute per-step duration', () => {
      ...
      assert.strictEqual(result.steps[0].step, '1_ticket');
      assert.strictEqual(result.steps[0].duration, '60s');
      assert.strictEqual(result.steps[0].commandCount, 1);
      assert.strictEqual(result.steps[1].step, '2_bootstrap');
    ...
    it('should identify bottleneck step', () => {
      const actions = [
        ...
        { step: '5_implement', timestamp: '2026-02-26T20:00:10.000Z', what: 'step started' },
        { step: '5_implement', timestamp: '2026-02-26T20:10:10.000Z', what: 'step completed' },
      ];

      const result = analyzeActions(actions);
      assert.strictEqual(result.bottleneck, '5_implement');
      assert.strictEqual(result.bottleneckDuration, '600s');
    });

    it('should count blocks and retries', () => {
      const actions = [
        { step: '8_check', timestamp: '2026-02-26T20:00:00.000Z', what: 'step started' },
        { step: '8_check', timestamp: '2026-02-26T20:01:00.000Z', what: 'BLOCKED: Skill(check) not in_progress' },
        { step: '8_check', timestamp: '2026-02-26T20:02:00.000Z', what: 'BLOCKED: transition without evidence' },
        { step: '8_check', timestamp: '2026-02-26T20:03:00.000Z', what: 'step reset' },
        { step: '8_check', timestamp: '2026-02-26T20:05:00.000Z', what: 'Skill(check)' },
        { step: '8_check', timestamp: '2026-02-26T20:10:00.000Z', what: 'step completed' },
      ];

      const result = analyzeActions(actions);
      const checkStep = result.steps.find(s => s.step === '8_check');
    ...
    it('should compute total duration from first to last action', () => {
      const actions = [
        ...
        { step: '15_complete', timestamp: '2026-02-26T20:44:00.000Z', what: 'step completed' },
      ];
