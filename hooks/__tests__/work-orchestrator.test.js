...
describe('work-orchestrator.js', () => {
  ...
  describe('plan command', () => {
    ...
    it('should detect ticket format correctly', async () => {
      const { result: ticketResult } = await runOrchestrator(['APPSUPEN-123']);
      expect(ticketResult.ticket).toBe('APPSUPEN-123');
  ...
  describe('ticket format handling', () => {
    it('should accept uppercase ticket IDs', async () => {
      const { result } = await runOrchestrator(['APPSUPEN-123']);
      expect(result.ticket).toBe('APPSUPEN-123');
    });
