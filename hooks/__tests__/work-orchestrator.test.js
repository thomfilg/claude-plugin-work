...
// ─── parseTicketInput unit tests (GH-146) ──────────────────────────────────

const { parseTicketInput } = require('../work-orchestrator');

describe('parseTicketInput (GH-146)', () => {
  it('returns ticketBase as-is for plain ticket IDs', () => {
    const r = parseTicketInput('PROJ-123');
    assert.equal(r.ticketBase, 'PROJ-123');
    assert.equal(r.suffix, null);
  });

  it('parses hyphenated suffix: JUL-1397-bugfix', () => {
    const r = parseTicketInput('JUL-1397-bugfix');
    assert.equal(r.ticketBase, 'JUL-1397');
    assert.equal(r.suffix, 'bugfix');
    assert.equal(r.separator, '-');
  });

  it('parses hyphenated suffix: APPSUPEN-867-monitoring', () => {
    const r = parseTicketInput('APPSUPEN-867-monitoring');
    assert.equal(r.ticketBase, 'APPSUPEN-867');
    assert.equal(r.suffix, 'monitoring');
    assert.equal(r.separator, '-');
  });

  it('parses slash suffix: GH-145/phase1', () => {
    const r = parseTicketInput('GH-145/phase1');
    assert.equal(r.ticketBase, 'GH-145');
    assert.equal(r.suffix, 'phase1');
    assert.equal(r.separator, '/');
  });

  it('parses slash suffix: PROJ-99/hotfix', () => {
    const r = parseTicketInput('PROJ-99/hotfix');
    assert.equal(r.ticketBase, 'PROJ-99');
    assert.equal(r.suffix, 'hotfix');
    assert.equal(r.separator, '/');
  });

  it('returns null suffix for URLs', () => {
    const r = parseTicketInput('https://github.com/org/repo/issues/42');
    assert.equal(r.suffix, null);
    assert.ok(r.ticketBase.startsWith('https://'));
  });

  it('returns null suffix for null/undefined/empty', () => {
    assert.equal(parseTicketInput(null).suffix, null);
    assert.equal(parseTicketInput(undefined).suffix, null);
    assert.equal(parseTicketInput('').suffix, null);
  });

  it('throws on invalid suffix characters via slash', () => {
    assert.throws(() => parseTicketInput('PROJ-123/bad path!'), /invalid suffix/);
  });

  it('does not parse bare numbers as ticket/suffix', () => {
    const r = parseTicketInput('123/phase1');
    assert.equal(r.ticketBase, '123/phase1');
    assert.equal(r.suffix, null);
  });

  it('preserves case of suffix (critical for folder matching)', () => {
    const r = parseTicketInput('JUL-1397-bugfix');
    assert.equal(r.suffix, 'bugfix');  // NOT "BUGFIX"
  });
});
