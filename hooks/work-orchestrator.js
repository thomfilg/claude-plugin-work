...
// Parses "JUL-1397-bugfix" into { ticketBase: "JUL-1397", suffix: "bugfix", separator: "-" }.
...
function parseTicketInput(raw) {
  ...
  // Try hyphenated suffix: PROJ-123-suffix → ticketBase="PROJ-123", suffix="suffix"
  // Match: letters-digits-remainder (e.g., JUL-1397-bugfix, APPSUPEN-867-monitoring)
  // Uses '-' as separator in paths (tasks/JUL-1397-bugfix) instead of '/' (tasks/JUL-1397/phase1)
...
function inspect(ticket, providerConfig, suffix, suffixSeparator) {
  const s = {};
  const safeBase = tp.sanitizeTicketIdForPath(ticket, providerConfig);
  const sep = suffixSeparator || '/';
  const safeName = suffix ? safeBase + sep + suffix : safeBase;
...
function generatePlan(ticket, description, s, rework, callerProviderCfg, suffix, suffixSeparator) {
  const plan = [];
...
function main() {
  ...
  switch (command) {
    case 'plan': {
      ...
      // Parse suffix/phase syntax (e.g., "GH-145/phase1" or "JUL-1397-bugfix")
      let suffix = null;
      let suffixSeparator = '/';
      try {
        ...
        suffixSeparator = parsed.separator || '/';
      } catch (err) {
      ...
      const state = ticket ? inspect(ticket, providerConfig, suffix, suffixSeparator) : null;
      const result = generatePlan(ticket, isTicket ? null : raw, state, rework, providerConfig, suffix, suffixSeparator);
    ...
    case 'transition': {
      ...
      const transSep = transParsed.separator || '/';
      const safeTransTicket = tp.sanitizeTicketIdForPath(transBase, transProviderCfg) + (transParsed.suffix ? transSep + transParsed.suffix : '');
    ...
    case 'transitions': {
      ...
      const transitionsBase = transitionsProviderCfg?.provider === 'github' ? transParsed2.ticketBase : transParsed2.ticketBase.toUpperCase();
      const transitionsSep = transParsed2.separator || '/';
      const safeTransitionsTicket = tp.sanitizeTicketIdForPath(transitionsBase, transitionsProviderCfg) + (transParsed2.suffix ? transitionsSep + transParsed2.suffix : '');
    ...
    case 'actions': {
      ...
      const actionsSep = actionsParsed.separator || '/';
      const ticket = tp.sanitizeTicketIdForPath(actionsBase, actionsProviderCfg) + (actionsParsed.suffix ? actionsSep + actionsParsed.suffix : '');
    ...
    case 'record-tdd': {
      ...
      const tddSep = tddParsed.separator || '/';
      const ticket = tp.sanitizeTicketIdForPath(tddBase, tddProviderCfg) + (tddParsed.suffix ? tddSep + tddParsed.suffix : '');
      const stepId = rest[1];
