...
```bash
# Import from package
grep -r "from '@$my_repository_main_worktree_folder/shared-ui'" apps/$APP/app --include="*.tsx" | grep "$COMPONENT"
grep -r "from '@$my_repository_main_worktree_folder/ui'" apps/$APP/app --include="*.tsx" | grep "$COMPONENT"
...
- `status-site` → `tasks/APPSUPEN-XXX/screenshots/status-site/`
- `as-dashboard` → `tasks/APPSUPEN-XXX/screenshots/as-dashboard/`
