...
describe('enforce-step-workflow', () => {
  ...
  describe('Vector 3 exempt scripts', () => {
    ...
    it('blocks exempt script name from untrusted path', async () => {
      ...
      // Create a fake work.workflow.js in /tmp (untrusted)
      const fakePath = path.join(os.tmpdir(), 'work.workflow.js');
      fs.writeFileSync(
        fakePath,
    ...
    it('still blocks non-exempt script that references protected files', async () => {
      ...
      const { code, stderr } = await runHook(
        {
          tool_name: 'Bash',
          tool_input: { command: 'echo "work.workflow.js" > /tmp/.work-state.json' },
        },
        'PreToolUse'
      );
