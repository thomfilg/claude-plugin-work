 ...
 * tasks-next.js
 *
 * Self-paced runner for the `tasks` step. Mirrors brief-next.js / spec-next.js.
 * Phases: inputs → requirements_extract → draft → traceability → kind_assign →
...
const TASKS_PHASE_CLI = path.resolve(__dirname, 'tasks-phase-state.js');

const { TASKS_INITIAL_PHASE } = require('./tasks-phase-registry');
...
function die(msg) {
  process.stderr.write(`tasks-next: ${msg}\n`);
  process.exit(2);
}
...
function callPhaseCli(args) {
  mintCompanionToken();
  const r = spawnSync(process.execPath, [TASKS_PHASE_CLI, ...args], {
    encoding: 'utf8',
    stdio: 'pipe',
  });
...
function main(argv) {
  ...
  if (!ticket || /^-/.test(ticket)) {
    process.stderr.write('usage: tasks-next.js <TICKET>\n  e.g. node tasks-next.js ECHO-4579\n');
    process.exit(2);
  }
  ...
  snapshotCompanionToken('tasks-phase-state.js', ticket);
  ...
  ensureInit(ticket);
  let phase = getCurrentPhase(ticket) || TASKS_INITIAL_PHASE;
