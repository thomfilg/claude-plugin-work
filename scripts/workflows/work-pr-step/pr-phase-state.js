 ...
 * pr-phase-state.js
 *
 * Authorized writer for `tasks/<ticket>/pr-phase.json`. Mirrors the
 * agent-gated pattern used by brief-phase-state.js / spec-phase-state.js.
 *
 * Allow-list: `pr-generator` (the user-facing skill that drives the
 * `tasks` step). Any sub-agent the skill spawns will be normalized to
 * the skill name via CLAUDE_CURRENT_AGENT.
 *
 * Subcommands:
 *   node pr-phase-state.js init       <TICKET>
 *   node pr-phase-state.js current    <TICKET>
 *   node pr-phase-state.js record     <TICKET> <phase> [--summary "..."]
 *   node pr-phase-state.js transition <TICKET> <target>
 */
...
const ALLOWED_AGENTS = ['pr-generator', 'pr-post-generator'];
const GATED_SUBCOMMANDS = ['init', 'record', 'transition'];
const TOKEN_MAX_AGE_MS = 10_000;

const PHASES = PR_PHASE_ORDER;
...
function cmdRecord(ticket, phase, args) {
  if (!isPrPhase(phase)) {
    errorExit(`Unknown phase "${phase}". Valid: ${PR_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No pr-phase state for ${ticket}. Run \`init\` first.`);
...
function cmdTransition(ticket, target) {
  if (!isPrPhase(target)) {
    errorExit(`Unknown target phase "${target}". Valid: ${PR_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No pr-phase state for ${ticket}. Run \`init\` first.`);
  if (!prCanTransition(state.currentPhase, target)) {
    const allowed = prNextPhases(state.currentPhase);
    errorExit(
...
function main(argv) {
  ...
  const sub = args[0];
  if (!sub) {
    errorExit('Usage: pr-phase-state.js <init|current|record|transition> <TICKET> [args]');
  }
  const ticket = args[1];
  ...
  if (sub === 'record') {
    const phase = args[2];
    if (!phase) errorExit('Usage: pr-phase-state.js record <TICKET> <phase> [--summary "..."]');
    return cmdRecord(ticket, phase, args.slice(3));
  }
  if (sub === 'transition') {
    const target = args[2];
    if (!target) errorExit('Usage: pr-phase-state.js transition <TICKET> <target>');
...
module.exports = { PHASES, prCanTransition, prNextPhases };
