...
const PR_PHASE_CLI = path.resolve(__dirname, 'pr-phase-state.js');
...
function die(msg) {
  process.stderr.write(`pr-next: ${msg}\n`);
  process.exit(2);
}
...
function callPhaseCli(args) {
  mintCompanionToken();
  const r = spawnSync(process.execPath, [PR_PHASE_CLI, ...args], {
    encoding: 'utf8',
    stdio: 'pipe',
  });
...
function main(argv) {
  ...
  if (!ticket || /^-/.test(ticket)) {
    process.stderr.write('usage: pr-next.js <TICKET>\n  e.g. node pr-next.js ECHO-4579\n');
    process.exit(2);
  }
  logNextScriptEvent('pr-next', {
    event: 'invoked',
    ticket,
    cwd: process.cwd(),
  ...
  snapshotCompanionToken('pr-phase-state.js', ticket);
  ...
  ensureInit(ticket);
  let phase = getCurrentPhase(ticket) || PR_INITIAL_PHASE;
