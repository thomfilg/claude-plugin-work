 ...
 *
 * Authorized writer for `tasks/<ticket>/task-review-phase.json`. Mirrors
 * pr-review-phase-state.js. Only callable through Claude Code's agent
 * system (token-protected), and only by the task-reviewer (or code-checker
 * acting in the task_review step) agent.
 */
...
const ALLOWED_AGENTS = ['task-reviewer', 'code-checker'];
const GATED_SUBCOMMANDS = ['init', 'record', 'transition'];
const TOKEN_MAX_AGE_MS = 10_000;

const PHASES = TASK_REVIEW_PHASE_ORDER;
...
function getStatePath(ticketId) {
  ...
  const resolved = path.resolve(base, safeId, 'task-review-phase.json');
...
function cmdInit(ticket) {
  ...
  const state = {
    ...
    createdAt: now,
    updatedAt: now,
    currentPhase: TASK_REVIEW_INITIAL_PHASE,
    phases: {},
  };
...
function cmdRecord(ticket, phase, args) {
  if (!isTaskReviewPhase(phase)) {
    errorExit(`Unknown phase "${phase}". Valid: ${TASK_REVIEW_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No task-review-phase state for ${ticket}. Run \`init\` first.`);
...
function cmdTransition(ticket, target) {
  if (!isTaskReviewPhase(target)) {
    errorExit(`Unknown target phase "${target}". Valid: ${TASK_REVIEW_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No task-review-phase state for ${ticket}. Run \`init\` first.`);
  if (!taskReviewCanTransition(state.currentPhase, target)) {
    const allowed = taskReviewNextPhases(state.currentPhase);
    errorExit(
...
function main(argv) {
  ...
  const sub = args[0];
  if (!sub) {
    errorExit('Usage: task-review-phase-state.js <init|current|record|transition> <TICKET> [args]');
  }
  const ticket = args[1];
  ...
  if (sub === 'record') {
    const phase = args[2];
    if (!phase)
      errorExit('Usage: task-review-phase-state.js record <TICKET> <phase> [--summary "..."]');
    return cmdRecord(ticket, phase, args.slice(3));
  }
  if (sub === 'transition') {
    const target = args[2];
    if (!target) errorExit('Usage: task-review-phase-state.js transition <TICKET> <target>');
...
module.exports = { PHASES, taskReviewCanTransition, taskReviewNextPhases };
