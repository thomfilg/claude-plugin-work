 ...
 * task-review-next.js
 *
 * Self-paced runner for the task-reviewer / code-checker agent during the
 * `task_review` step. Mirrors pr-review-next.js. Phases:
...
const TASK_REVIEW_PHASE_CLI = path.resolve(__dirname, 'task-review-phase-state.js');
...
function die(msg) {
  process.stderr.write(`task-review-next: ${msg}\n`);
  process.exit(2);
}
...
function callPhaseCli(args) {
  mintCompanionToken();
  const r = spawnSync(process.execPath, [TASK_REVIEW_PHASE_CLI, ...args], {
    encoding: 'utf8',
    stdio: 'pipe',
  });
...
function ensureInit(ticket) {
  ...
  if (r.code !== 0) die(`Could not init task-review-phase state:\n${r.out}`);
}
...
function main(argv) {
  ...
  if (!ticket || /^-/.test(ticket)) {
    process.stderr.write(
      'usage: task-review-next.js <TICKET>\n  e.g. node task-review-next.js ECHO-4579\n'
    );
    process.exit(2);
  }
  logNextScriptEvent('task-review-next', {
    event: 'invoked',
    ticket,
    cwd: process.cwd(),
  ...
  snapshotCompanionToken('task-review-phase-state.js', ticket);
  ...
  ensureInit(ticket);
  let phase = getCurrentPhase(ticket) || TASK_REVIEW_INITIAL_PHASE;
