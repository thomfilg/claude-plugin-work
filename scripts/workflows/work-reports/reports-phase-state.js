 ...
 *
 * Authorized writer for `tasks/<ticket>/reports-phase.json`. Mirrors
 * pr-review-phase-state.js. Only callable through Claude Code's agent
 * system, and only by the reports-writer agent.
 */
...
function getStatePath(ticketId) {
  ...
  const resolved = path.resolve(base, safeId, 'reports-phase.json');
...
function cmdInit(ticket) {
  ...
  const state = {
    ...
    createdAt: now,
    updatedAt: now,
    currentPhase: REPORTS_INITIAL_PHASE,
    phases: {},
  };
...
function cmdRecord(ticket, phase, args) {
  if (!isReportsPhase(phase)) {
    errorExit(`Unknown phase "${phase}". Valid: ${REPORTS_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No reports-phase state for ${ticket}. Run \`init\` first.`);
...
function cmdTransition(ticket, target) {
  if (!isReportsPhase(target)) {
    errorExit(`Unknown target phase "${target}". Valid: ${REPORTS_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No reports-phase state for ${ticket}. Run \`init\` first.`);
  if (!reportsCanTransition(state.currentPhase, target)) {
    const allowed = reportsNextPhases(state.currentPhase);
    errorExit(
...
function main(argv) {
  ...
  const sub = args[0];
  if (!sub) {
    errorExit('Usage: reports-phase-state.js <init|current|record|transition> <TICKET> [args]');
  }
  const ticket = args[1];
  ...
  if (sub === 'record') {
    const phase = args[2];
    if (!phase)
      errorExit('Usage: reports-phase-state.js record <TICKET> <phase> [--summary "..."]');
    return cmdRecord(ticket, phase, args.slice(3));
  }
  if (sub === 'transition') {
    const target = args[2];
    if (!target) errorExit('Usage: reports-phase-state.js transition <TICKET> <target>');
...
module.exports = { PHASES, reportsCanTransition, reportsNextPhases };
