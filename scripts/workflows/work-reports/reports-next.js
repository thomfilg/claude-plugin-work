 ...
 * reports-next.js
 *
 * Self-paced runner for the reports-writer agent during the `reports`
 * step. Mirrors pr-review-next.js. Phases:
...
const REPORTS_PHASE_CLI = path.resolve(__dirname, 'reports-phase-state.js');

const { REPORTS_INITIAL_PHASE } = require('./reports-phase-registry');
...
function die(msg) {
  process.stderr.write(`reports-next: ${msg}\n`);
  process.exit(2);
}
...
function callPhaseCli(args) {
  mintCompanionToken();
  const r = spawnSync(process.execPath, [REPORTS_PHASE_CLI, ...args], {
    encoding: 'utf8',
    stdio: 'pipe',
  });
...
function ensureInit(ticket) {
  ...
  if (r.code !== 0) die(`Could not init reports-phase state:\n${r.out}`);
}
...
function main(argv) {
  ...
  if (!ticket || /^-/.test(ticket)) {
    process.stderr.write(
      'usage: reports-next.js <TICKET>\n  e.g. node reports-next.js ECHO-4579\n'
    );
    process.exit(2);
  }
  logNextScriptEvent('reports-next', {
    event: 'invoked',
    ticket,
    cwd: process.cwd(),
  ...
  snapshotCompanionToken('reports-phase-state.js', ticket);
  ...
  ensureInit(ticket);
  let phase = getCurrentPhase(ticket) || REPORTS_INITIAL_PHASE;
