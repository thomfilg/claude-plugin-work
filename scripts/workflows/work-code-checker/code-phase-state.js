 ...
 * code-phase-state.js
 *
 * Authorized writer for `tasks/<ticket>/code-phase.json`. Mirrors
 * spec-phase-state.js / completion-phase-state.js. Only callable through
 * Claude Code's agent system (token-protected), and only by the
 * code-checker agent during the `check` step.
 */
...
function getStatePath(ticketId) {
  ...
  const resolved = path.resolve(base, safeId, 'code-phase.json');
...
function cmdInit(ticket) {
  ...
  const state = {
    ...
    createdAt: now,
    updatedAt: now,
    currentPhase: CODE_INITIAL_PHASE,
    phases: {},
  };
...
function cmdRecord(ticket, phase, args) {
  if (!isCodePhase(phase)) {
    errorExit(`Unknown phase "${phase}". Valid: ${CODE_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No code-phase state for ${ticket}. Run \`init\` first.`);
...
function cmdTransition(ticket, target) {
  if (!isCodePhase(target)) {
    errorExit(`Unknown target phase "${target}". Valid: ${CODE_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No code-phase state for ${ticket}. Run \`init\` first.`);
  if (!codeCanTransition(state.currentPhase, target)) {
    const allowed = codeNextPhases(state.currentPhase);
    errorExit(
...
function main(argv) {
  ...
  const sub = args[0];
  if (!sub) {
    errorExit('Usage: code-phase-state.js <init|current|record|transition> <TICKET> [args]');
  }
  const ticket = args[1];
  ...
  if (sub === 'record') {
    const phase = args[2];
    if (!phase) errorExit('Usage: code-phase-state.js record <TICKET> <phase> [--summary "..."]');
    return cmdRecord(ticket, phase, args.slice(3));
  }
  if (sub === 'transition') {
    const target = args[2];
    if (!target) errorExit('Usage: code-phase-state.js transition <TICKET> <target>');
...
module.exports = { PHASES, codeCanTransition, codeNextPhases };
