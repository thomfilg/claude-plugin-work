...
const CI_PHASE_CLI = path.resolve(__dirname, 'ci-phase-state.js');

const { CI_INITIAL_PHASE } = require('./ci-phase-registry');
...
function die(msg) {
  process.stderr.write(`ci-next: ${msg}\n`);
  process.exit(2);
}
...
function ensureInit(ticket) {
  ...
  if (r.code !== 0) die(`Could not init ci-phase state:\n${r.out}`);
}
...
function main(argv) {
  ...
  if (!ticket || /^-/.test(ticket)) {
    process.stderr.write('usage: ci-next.js <TICKET>\n  e.g. node ci-next.js ECHO-4579\n');
    process.exit(2);
  }
  logNextScriptEvent('ci-next', {
    event: 'invoked',
    ticket,
    cwd: process.cwd(),
  ...
  ensureInit(ticket);
  let phase = getCurrentPhase(ticket) || CI_INITIAL_PHASE;
