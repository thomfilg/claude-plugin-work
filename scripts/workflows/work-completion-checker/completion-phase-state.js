 ...
 *
 * Authorized writer for `tasks/<ticket>/completion-phase.json`. Mirrors
 * the agent-gated pattern used by spec-phase-state.js / brief-phase-state.js:
 * only callable through Claude Code's agent system (token-protected),
 * and only by the completion-checker agent during the `check` step.
 *
 * Subcommands:
 *   node completion-phase-state.js init       <TICKET>
 *   node completion-phase-state.js current    <TICKET>
 *   node completion-phase-state.js record     <TICKET> <phase> [--summary "..."]
 *   node completion-phase-state.js transition <TICKET> <target>
 */
...
function getStatePath(ticketId) {
  ...
  const resolved = path.resolve(base, safeId, 'completion-phase.json');
...
function cmdInit(ticket) {
  ...
  const state = {
    ...
    createdAt: now,
    updatedAt: now,
    currentPhase: COMPLETION_INITIAL_PHASE,
    phases: {},
  };
...
function cmdRecord(ticket, phase, args) {
  if (!isCompletionPhase(phase)) {
    errorExit(`Unknown phase "${phase}". Valid: ${COMPLETION_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No completion-phase state for ${ticket}. Run \`init\` first.`);
...
function cmdTransition(ticket, target) {
  if (!isCompletionPhase(target)) {
    errorExit(`Unknown target phase "${target}". Valid: ${COMPLETION_PHASE_ORDER.join(', ')}.`);
  }
  const state = readState(ticket);
  if (!state) errorExit(`No completion-phase state for ${ticket}. Run \`init\` first.`);
  if (!completionCanTransition(state.currentPhase, target)) {
    const allowed = completionNextPhases(state.currentPhase);
    errorExit(
...
function main(argv) {
  ...
  const sub = args[0];
  if (!sub) {
    errorExit('Usage: completion-phase-state.js <init|current|record|transition> <TICKET> [args]');
  }
  const ticket = args[1];
  ...
  if (sub === 'record') {
    const phase = args[2];
    if (!phase)
      errorExit('Usage: completion-phase-state.js record <TICKET> <phase> [--summary "..."]');
    return cmdRecord(ticket, phase, args.slice(3));
  }
  if (sub === 'transition') {
    const target = args[2];
    if (!target) errorExit('Usage: completion-phase-state.js transition <TICKET> <target>');
...
module.exports = { PHASES, completionCanTransition, completionNextPhases };
